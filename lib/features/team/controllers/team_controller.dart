...
class TeamController extends GetxController {
  ...
  Future<TeamModel?> getTeam(String teamId) async {
    final doc =
        await _db.collection(AppConstants.colTeams).doc(teamId).get();
    ...
    try {
      ...
      if (userQuery.docs.isNotEmpty) {
        ...
        await ownerDoc.reference.update({
          'groupRoles.$_groupId': AppConstants.roleOwner,
        });
      }
  ...
  Future<void> importTeamsFromExcel() async {
    try {
      ...
      for (int i = 1; i < sheet.maxRows; i++) {
        ...
        if (row.isEmpty || row[AppConstants.excelTeamName]?.value == null)
          continue;

        final name =
            row[AppConstants.excelTeamName]?.value?.toString().trim() ?? '';
        final ownerName =
            row[AppConstants.excelTeamOwnerName]?.value?.toString().trim() ??
                '';
        final ownerPhone =
            row[AppConstants.excelTeamOwnerPhone]?.value?.toString().trim() ??
                '';
        final ownerType =
            row[AppConstants.excelTeamOwnerType]?.value?.toString().trim() ??
        ...
        await addTeam(
          ...
          ownerAddress:
              row[AppConstants.excelTeamOwnerAddress]?.value?.toString(),
          ownerType: ownerType,
          ownerLastTeam:
              row[AppConstants.excelTeamLastTeam]?.value?.toString(),
        );
        added++;
      }
  ...
  Future<void> recordPlayerSold(
    ...
    await _db.collection(AppConstants.colTeams).doc(teamId).update({
