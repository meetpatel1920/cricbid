...
class PlayerController extends GetxController {
  ...
  Future<void> addPlayer({
    ...
    try {
      ...
      final player = PlayerModel(
        ...
        playerNumber: number,
        auctionStatus: AppConstants.playerStatusPending,
      ...
      if (userQuery.docs.isNotEmpty) {
        ...
        if (existingRole.isEmpty) {
          await userDoc.reference.update({
            'groupRoles.$_groupId': AppConstants.rolePlayer,
          });
        }
      }
  ...
  Future<void> importPlayersFromExcel() async {
    try {
      ...
      for (int i = 1; i < sheet.maxRows; i++) {
        ...
        if (row.isEmpty || row[AppConstants.excelPlayerName]?.value == null)
          continue;

        final name =
            row[AppConstants.excelPlayerName]?.value?.toString().trim() ?? '';
        final phone =
            row[AppConstants.excelPlayerPhone]?.value?.toString().trim() ?? '';
        final type =
            row[AppConstants.excelPlayerType]?.value?.toString().trim() ??
                AppConstants.typeBatting;
        final imageUrl =
            row[AppConstants.excelPlayerImageUrl]?.value?.toString().trim();
        ...
        await addPlayer(
          ...
          phone: phone,
          address:
              row[AppConstants.excelPlayerAddress]?.value?.toString(),
          type: type,
          lastTeam:
              row[AppConstants.excelPlayerLastTeam]?.value?.toString(),
          photoUrl: imageUrl,
        );
        added++;
      }
  ...
  Future<String?> _uploadImage(String localPath, String playerId) async {
    try {
      ...
      final ref = _storage
          .ref()
          .child(AppConstants.storagePlayerImages)
          .child('$playerId.jpg');
  ...
  List<PlayerModel> get unsoldPlayers => players
      .where((p) =>
          p.auctionStatus == AppConstants.playerStatusUnsold ||
          p.auctionStatus == AppConstants.playerStatusSkipped ||
          p.auctionStatus == AppConstants.playerStatusPending)
      .toList();

  List<PlayerModel> get soldPlayers => players
      .where((p) => p.auctionStatus == AppConstants.playerStatusSold)
      .toList();
