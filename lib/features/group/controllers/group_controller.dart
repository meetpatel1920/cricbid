...
class GroupController extends GetxController {
  ...
  Future<void> _loadCurrentGroup() async {
    ...
    final doc =
        await _db.collection(AppConstants.colGroups).doc(id).get();
    if (doc.exists) {
  ...
  Future<void> createGroup({
    ...
    try {
      ...
      await _db.collection(AppConstants.colUsers).doc(user.uid).update({
        'groupRoles.$groupId': AppConstants.roleAdmin,
      });

      _authCtrl.currentUser.value = user.copyWith(
        groupRoles: {...user.groupRoles, groupId: AppConstants.roleAdmin},
      );
  ...
  Future<void> updateGroupSettings({
    ...
    await _db.collection(AppConstants.colGroups).doc(groupId).update(updates);
