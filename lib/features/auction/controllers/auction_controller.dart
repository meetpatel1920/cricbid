...
class AuctionController extends GetxController {
  ...
  Future<void> _loadCurrentAuction() async {
    ...
    final doc = await _db
        .collection(AppConstants.colAuctions)
  ...
  Future<void> _loadRounds(String auctionId) async {
    final snap = await _db
        .collection(AppConstants.colAuctionRounds)
  ...
  void _loadEventHistory() {
    if (_groupId.isEmpty) return;
    _db
        .collection(AppConstants.colBids)
  ...
  Future<void> startAuction() async {
    ...
    try {
      ...
      final auction = AuctionModel(
        id: auctionId,
        groupId: _groupId,
        status: AppConstants.auctionStatusLive,
        currentRound: 1,
  ...
  Future<void> markSold({
    ...
    try {
      // Update player
      await _db.collection(AppConstants.colPlayers).doc(playerId).update({
        'auctionStatus': AppConstants.playerStatusSold,
        'teamId': teamId,
        'teamName': teamName,
        'soldPoints': points,
      });
  ...
  Future<void> skipPlayer(String playerId) async {
    await _db.collection(AppConstants.colPlayers).doc(playerId).update({
      'auctionStatus': AppConstants.playerStatusSkipped,
    });
  ...
  Future<void> _advanceToNextPlayer() async {
    ...
    if (nextIndex >= round.playerIds.length) {
      ...
      await _db
          ...
          .update({'status': AppConstants.auctionStatusPaused});
  ...
  Future<void> resumeAuction() async {
    await _db
        .collection(AppConstants.colAuctions)
        .doc(currentAuction.value?.id)
        .update({'status': AppConstants.auctionStatusLive});
    await _db
        .collection(AppConstants.colGroups)
        .doc(_groupId)
        .update({'isAuctionLive': true});
  ...
  Future<void> _completeAuction(String auctionId) async {
    await _db
        ...
        .doc(auctionId)
        .update({
      'status': AppConstants.auctionStatusCompleted,
