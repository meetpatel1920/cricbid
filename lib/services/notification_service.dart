...
class NotificationService extends GetxService {
  ...
  void _saveFcmTokenToAuth(String token) {
    ...
    _db.collection(AppConsts.colUsers).where('fcmToken', isNull: true).limit(1).get().then((_) {});
  ...
  void _handleForegroundMessage(RemoteMessage message) {
    ...
    String channelId = AppConsts.notifChannelGeneral;
    if (type.contains('auction')) channelId = AppConsts.notifChannelAuction;
    if (type.contains('match')) channelId = AppConsts.notifChannelMatch;
    if (type.contains('chat')) channelId = AppConsts.notifChannelChat;
  ...
  Future<void> sendGroupNotification({
    ...
    await _db.collection(AppConsts.colNotifications).add({
      'groupId': groupId,
      'title': title,
      'body': body,
      'type': type,
    ...
    final usersSnap = await _db.collection(AppConsts.colUsers).where('groupRoles.$groupId', isNotEqualTo: null).get();
  ...
  Future<void> sendUserNotification({
    ...
    final userDoc = await _db.collection(AppConsts.colUsers).doc(uid).get();
  ...
  void listenForPersonalNotifications(String uid) {
    _db.collection(AppConsts.colNotifications).where('uid', isEqualTo: uid).where('shown', isNotEqualTo: true).snapshots().listen((snap) {
  ...
  void listenForGroupNotifications(String groupId) {
    ...
    _db
        .collection(AppConsts.colNotifications)
      ...
      for (final change in snap.docChanges) {
        if (change.type == DocumentChangeType.added) {
          ...
          String channelId = AppConsts.notifChannelAuction;
          if (type.contains('match')) channelId = AppConsts.notifChannelMatch;
...
//   }) async {
//     final userDoc =
//         await _db.collection(AppConsts.colUsers).doc(uid).get();
//     if (!userDoc.exists) return;
